...
| 对象 | 说明 |
| --- | --- |
| `env` | 当前环境配置，包含 `name`、`base_url`、`headers`、`variables`，以及可选的 `runtime` / `readiness` / `logs` |
...
按 DSL 安排一次“稍后由 test-runner / mock 主动去调用被测系统”的 HTTP callback：
...
- `after_ms` 可选，默认 `0`
- `request.api` 必填；callback 会在入队时先把模板解析成具体请求，再交给异步调度器
- callback step 成功表示“已成功入队”，真正的投递结果会出现在最终报告里的 `callbacks` 列表中

step 结束后，`result` 会被写成一条 enqueue 记录，例如：
...
::: tip Callback 专题
如果你想看 callback 在 case、mock route、workflow 三个层面的配合方式，以及 `sample-projects/` 里的完整样例，请继续阅读 [Callback](/guide/callbacks)。
:::
...
- `steps`：可选，目前支持 `set`、`callback` 和 `if`
- `respond`：可选，定义动态响应
...
如果你要模拟“第三方同步受理、随后异步回调被测系统”，可以直接在 mock route 里安排 callback：
...
- `steps` 目前不支持 `request`、`sql`、`redis`、`query_db`、`query_redis`、`sleep`、`foreach`
